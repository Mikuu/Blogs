...
在consumer端启动一个mock server（如果你有看源码，它就是一个普通的HttpServer实例），consumer向这个mock server发送request获取response，
整个过程被记录成JSON的契约文件。
...
因为，某个服务经常既是consumer，又是provider）。而契约测试的被测对象，一定是provider。好了，这就是问题的核心，其它的细节，就不必再赘述了。
